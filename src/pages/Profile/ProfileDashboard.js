...
export default function ProfileDashboard() {
    ...
    if (loading && !profileFields.email) {
        return (_jsxs(DashboardShell, { sidebarLinks: sidebarLinks, title: "Seekers Dashboard", subtitle: "Loading...", hideNavigation: true, children: [_jsx("div", { className: "flex h-64 items-center justify-center", children: _jsx("p", { className: "text-gray-500", children: "Loading profile data..." }) }), _jsx(Modal, { open: showApiKeyModal, onClose: () => setShowApiKeyModal(false), title: "Enter Google Gemini API Key", maxWidth: "max-w-md", children: _jsxs("div", { className: "space-y-4", children: [_jsxs("p", { className: "text-sm text-gray-600", children: ["To use the Resume Scanner, please provide a free Google Gemini API Key. You can get one from ", _jsx("a", { href: "https://aistudio.google.com/app/apikey", target: "_blank", rel: "noreferrer", className: "text-brand underline", children: "Google AI Studio" }), "."] }), _jsx(Input, { placeholder: "AIzaSy...", value: apiKey, onChange: (e) => setApiKey(e.target.value), type: "password" }), _jsxs("div", { className: "flex justify-end gap-2", children: [_jsx(Button, { variant: "ghost", onClick: () => setShowApiKeyModal(false), children: "Cancel" }), _jsx(Button, { variant: "primary", disabled: !apiKey || !tempAnalyzeFile, onClick: () => tempAnalyzeFile && handleResumeAnalysis(apiKey, tempAnalyzeFile), children: "Analyze Resume" })] })] }) })] }));
    }
    return (_jsxs(DashboardShell, {
        sidebarLinks: sidebarLinks, title: "Seekers Dashboard", subtitle: "Manage your dental profile, resumes, and saved jobs.", hideNavigation: true, children: [_jsx(Breadcrumbs, { items: [{ label: 'Seeker Home', to: '/seekers' }, { label: 'Seekers Dashboard' }] }), _jsx(Tabs, {
            tabs: [
                { id: 'profile', label: 'Profile' },
                { id: 'applications', label: 'Applications' },
                { id: 'saved', label: 'Saved jobs' }
            ], active: activeTab, onChange: setActiveTab
        }), activeTab === 'profile' && (_jsxs("div", {
            className: "rounded-2xl border border-gray-100 bg-white p-5 shadow-sm space-y-6", children: [_jsxs("div", {
                className: "rounded-2xl border border-dashed border-black bg-white p-4 shadow-sm", children: [_jsxs("div", {
                    className: "flex items-center justify-between", children: [_jsxs("div", { children: [_jsx("p", { className: "text-lg font-semibold text-gray-900", children: "Upload a resume" }), _jsx("p", { className: "text-sm text-gray-500", children: "Upload your latest resume to be visible to employers." })] }), _jsx("label", {
                        className: "flex cursor-pointer items-center gap-2 rounded-lg bg-indigo-50 px-3 py-2 text-sm font-medium text-indigo-700 hover:bg-indigo-100 transition", children: isAnalyzing ? (_jsx("span", { children: "Analyzing..." })) : (_jsxs(_Fragment, {
                            children: [_jsx(ScanText, { className: "h-4 w-4" }), _jsx("span", { children: "Auto-fill from Resume" }), _jsx("input", {
                                type: "file", className: "hidden", accept: ".pdf", onChange: (e) => {
                                    ...
                                    app.status === 'Shortlisted' ? 'info' :
                                        app.status === 'Interview' ? 'warning' :
                                            app.status === 'Offer' ? 'success' :
                                                'default', children: app.status
                            })
                        })]
                    })
                }, app.id))))
            })]
        })), activeTab === 'saved' && (_jsxs("div", {
            className: "rounded-2xl border border-gray-100 bg-white p-5 shadow-sm", children: [_jsx("h3", { className: "text-lg font-semibold text-gray-900", children: "Saved jobs" }), _jsx("div", {
                className: "mt-3 grid gap-3", children: savedJobs.length === 0 ? (_jsx("p", { className: "text-sm text-gray-500", children: "No saved jobs yet." })) : (savedJobs.map((job) => (_jsx(JobCard, {
                    job: job, onApply: () => { }, isSaved: true, onToggleSave: async (j) => {
            ...
            className: "fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4 py-6", children: _jsxs("div", {
                className: "w-full max-w-md overflow-hidden rounded-2xl border border-gray-200 bg-white p-6 shadow-xl", children: [_jsxs("div", {
                    className: "flex items-center justify-between", children: [_jsx("h3", { className: "text-lg font-semibold text-gray-900", children: "Upload resume" }), _jsx("button", {
                        className: "text-xs font-semibold uppercase tracking-wide text-gray-500", onClick: () => {
                            setShowUploadModal(false);
                            setUploadFileName('');
                            setUploadFile(null);
                        }, children: "Close"
                    })]
                }), _jsx("p", { className: "mt-2 text-sm text-gray-600", children: "Upload your resume (.pdf, .docx). The new file will replace your current latest resume." }), _jsxs("label", {
                    className: "mt-4 flex cursor-pointer items-center justify-between gap-2 rounded-xl border border-dashed border-gray-300 bg-gray-50 px-4 py-3 text-sm font-semibold text-gray-700 transition hover:border-brand", children: [_jsxs("span", { className: "flex items-center gap-2", children: [_jsx(UploadCloud, { className: "h-4 w-4 text-brand" }), _jsx("span", { children: uploadFileName || 'Choose resume file' })] }), _jsx("input", {
                        type: "file", className: "hidden", accept: ".pdf,.docx,.doc", onChange: (event) => {
                    ...
                    className: "mt-4 flex justify-end gap-2", children: [_jsx(Button, {
                        variant: "ghost", size: "sm", onClick: () => {
                            setShowUploadModal(false);
                            setUploadFileName('');
                            setUploadFile(null);
                        }, children: "Cancel"
                    }), _jsx(Button, { variant: "primary", size: "sm", onClick: handleResumeUpload, disabled: !uploadFile || uploading, children: uploading ? 'Uploading...' : 'Upload' })]
                })]
            })
